...
void submitValues() {
  ...
  for (uint8_t timeout = 2; timeout != 0; timeout--) {
    ...
    if (connected == true) {
      Serial.print(F("Connection successful, transferring..."));
...
void setup() {
  ...
  Serial.begin(9600);
  while(!Serial);
  Serial.print(F("Initializing DHCP connection..."));
  if (Ethernet.begin(mac) == 0) {
    Serial.println(F("failed! Trying static IP setup."));
  ...
  Serial.println(F("Starting loop in 30 seconds."));
  HDC.getTemp();
  delay(30000);
}
